...
{
    [Info("Easter Panel", "MJSU", "1.0.0")]
    [Description("Displays if the easter event is active")]
    internal class EasterPanel : RustPlugin
    {
        #region Class Fields
        ...
        private PluginConfig AdditionalConfig(PluginConfig config)
        {
            config.Panel = new Panel
            {
                Image = new PanelImage
                {
                    ...
                    Url = config.Panel?.Image?.Url ?? "https://i.imgur.com/pTpHnl2.png",
        ...
        private void OnServerInitialized()
        {
            ...
            NextTick(() =>
            {
                _isEggHuntActive = UnityEngine.Object.FindObjectsOfType<EggHuntEvent>().Where(CanShowPanel).Any();
                if (_isEggHuntActive)
                {
        ...
        private Hash<string, object> GetPanel()
        {
            ...
            if (image != null)
            {
                image.Color = _isEggHuntActive ? _pluginConfig.ActiveColor : _pluginConfig.InactiveColor;
            }
